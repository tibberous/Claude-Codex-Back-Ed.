...
from PySide6.QtNetwork import QHostAddress, QTcpServer, QTcpSocket, QUdpSocket  # depcheck-ok
try:
...
# Inline DOM scraper for the chatgpt.com/library page used by the `library`
# bridge action (see SuperGrokBridgeWindow.fetchChatGptLibraryAsync).
# Returns a JSON string parseable by the Python callback.
...
def chatTargetFromUrl(url: object = "") -> str:
    ...
    if "claude.ai" in text:
        return "claude"
    if "copilot.microsoft.com" in text or "copilot.cloud.microsoft" in text or "bing.com/chat" in text:
        return "copilot"
    if "deepseek.com" in text or "chat.deepseek.com" in text:
        return "deepseek"
    return "grok"
...
class BridgeWebPage(QWebEnginePage):
    ...
    # Known-harmless noise emitted by third-party site code (Cloudflare turnstile,
    # Grok's own SVG/i18n bundle, Chromium permission-policy warnings). Still
    # captured via the consoleMessage signal for debug pane, but NOT forwarded
    # to stderr — keeps the operator's terminal readable.
    _CONSOLE_NOISE = (
    ...
    def javaScriptConsoleMessage(self, level: Any, message: str, lineNumber: int, sourceID: str) -> None:
        ...
        msg_str = str(message)
        if any(token in msg_str for token in self._CONSOLE_NOISE):
            return
        try:
...
def buildGrokDomSendScript(message: str, sendId: str, *, maxTicks: int = 300, stablePolls: int = 3, target: object = "grok") -> str:
  ...
  function __findEditorRow() {{
    ...
    /* Copilot (copilot.microsoft.com) uses cib-* web components for the chat
       surface. The composer is `cib-text-input` wrapping a textarea labeled
       userInput. Selectors tried in order; expect refinement after first
       round-trip — Copilot rotates DOM frequently. */
  ...
  function __messages() {{
    ...
    if (__target === 'copilot') {{
      /* Copilot (copilot.microsoft.com) — Bot turns are wrapped in
         cib-message[source="bot"] containers with cib-shared text blocks
         underneath. The Adaptive-Cards style .ac-textBlock is the actual
         text node. Selectors expected to need tuning after first round-trip;
         the cib-* shadow DOM has shifted across reskins. */
      collect([
        'cib-message[source="bot"] .ac-textBlock',
        'cib-message[source="bot"] cib-shared',
...
class GrokBridgeChatJob(QObject):
    ...
    def __init__(self, page: QWebEnginePage, message: str, debugPane: Any, timeoutSeconds: int, callback: Callable[[dict[str, Any]], None], parent: QObject | None = None, attachments: object = None, target: object = "grok") -> None:
        ...
        # GPT-vision pilot: last-resort autonomous login/recovery driver. Fires
        # AFTER maxSurfaceAttempts + maxReloadAttempts have all failed, NEVER on
        # the happy path. One attempt per chat job. See start.py _gptVisionPilot.
        self.pilotAttempted = False
    ...
    def handleSurfaceProbe(self, result: Any) -> None:
        ...
        if self.reloadAttempts < self.maxReloadAttempts:
            ...
            return
        # ----- GPT-vision pilot fallback -----
        # DOM probe has failed maxSurfaceAttempts times AND a full reload+reprobe
        # also failed. Before surrendering with revealAndFinish, hand the view
        # to the autonomous vision pilot (start.py:_gptVisionPilot). It drives
        # the page through login / consent / captcha screens, then we re-probe.
        # One pilot attempt per job; on failure we fall through to reveal.
        if not self.pilotAttempted:
            self.pilotAttempted = True
            self.log("warn", f"{self.sendId}: {self.providerLabel} DOM probe exhausted retries+reload; invoking GPT-vision pilot fallback")
            try:
                from start import _gptVisionPilot, GPT_VISION_PILOT_DEFAULT_VIEWPORT
                window = self.parent()
                view = getattr(window, "grokView", None) if window is not None else None
                if view is None:
                    self.log("warn", f"{self.sendId}: pilot unavailable -- no grokView accessible from job parent")
                else:
                    pilotResult = _gptVisionPilot(
                        view, self.page, self.target,
                        jobId=self.sendId,
                        viewport=GPT_VISION_PILOT_DEFAULT_VIEWPORT,
                        lastReason=str(probe.get("reason") or "prompt textarea not found"),
                    )
                    self.log("grokchat", f"{self.sendId}: pilot result ok={pilotResult.get('ok')} reason={pilotResult.get('reason')} steps={pilotResult.get('steps')}")
                    if pilotResult.get("ok"):
                        # Pilot reached the chat surface. Reset surface-probe
                        # counters and re-probe through the normal happy path.
                        self.surfaceAttempts = 0
                        self.reloadAttempts = self.maxReloadAttempts  # consume the reload budget
                        QTimer.singleShot(1500, self.probeBeforeSend)
                        return
                    # Pilot couldn't recover; fall through to reveal with a
                    # diagnostic reason so the operator knows GPT tried.
                    probe = dict(probe or {})
                    probe["pilotAttempted"] = True
                    probe["pilotReason"] = pilotResult.get("reason")
                    probe["pilotSteps"] = pilotResult.get("steps")
            except Exception as error:
                recordException("supergrok_bridge/app.py:GrokBridgeChatJob.handleSurfaceProbe.pilot", error, extra={"handler": "except Exception as error:"})
                self.log("warn", f"{self.sendId}: pilot crashed: {type(error).__name__}: {error}")
...
class BridgeCommandServer(QObject):
    ...
    def __init__(self, window: Any, port: int, parent: QObject | None = None) -> None:
        ...
        # Verbose mode: env SUPERGROK_VERBOSE=1 OR --debug at parent. Logs every
        # wire byte to stderr so curl/telnet/nc sessions are observable.
        self.verbose: bool = (
            str(os.environ.get("SUPERGROK_VERBOSE", "") or "").strip() in ("1", "true", "yes", "on")
            or bool(getattr(getattr(window, "config", None), "debug", False))
        )
        # Tracks which sockets are talking HTTP vs raw newline-JSON, so respond()
        # can frame the reply correctly.
        self.httpSockets: set[int] = set()

    def start(self) -> bool:
        if not self.server.listen(QHostAddress(QHostAddress.SpecialAddress.LocalHost), self.port):
            ...
            return False
        self.log("bridge-service", f"TCP listening on {BRIDGE_SERVICE_HOST}:{self.server.serverPort()} (curl/telnet/nc — auto-detects HTTP vs raw newline-JSON)")
        # Companion UDP listener on the same port. Datagrams must be single
        # complete JSON objects per packet (UDP is connectionless — no
        # framing). Reply unicast to the sender's address. Use `nc -u
        # 127.0.0.1 <port>` or any UDP client.
        self.udp = QUdpSocket(self)
        if self.udp.bind(QHostAddress(QHostAddress.SpecialAddress.LocalHost), int(self.server.serverPort())):
            self.udp.readyRead.connect(self._readUdpDatagrams)
            self.log("bridge-service", f"UDP listening on {BRIDGE_SERVICE_HOST}:{self.server.serverPort()} (one JSON object per datagram)")
        else:
            self.log("warn", f"UDP listener bind failed on {BRIDGE_SERVICE_HOST}:{self.server.serverPort()}: {self.udp.errorString()}")
        emitDebuggerHeartbeat(eventKind="bridge-service", reason="listening", caller="BridgeCommandServer.start", varDump={"port": int(self.server.serverPort()), "verbose": self.verbose})
        return True

    @Slot()
    def _readUdpDatagrams(self) -> None:
        """Consume every pending UDP datagram, parse as a single JSON action,
        reply unicast. Each datagram is independent (no buffering across
        datagrams the way TCP buffers across recv() calls)."""
        try:
            while self.udp.hasPendingDatagrams():
                size = int(self.udp.pendingDatagramSize())
                datagram, host, port = self.udp.readDatagram(size if size > 0 else 65535)
                raw = bytes(datagram).strip()
                if self.verbose:
                    print(f"[BRIDGE-SERVICE:verbose] udp recv from={host.toString()}:{port} bytes={len(raw)}  body={raw[:200]!r}", file=sys.stderr, flush=True)
                if not raw:
                    continue
                try:
                    ...
                    # Synchronous-only actions over UDP — chat job orchestration
                    # needs reliable delivery, so reject those.
                    if action == "chat":
                        payload = {"ok": False, "error": "chat actions are TCP-only (UDP can't reliably stream long jobs); use POST/HTTP or newline-JSON over TCP"}
                    else:
                        payload = self.buildSynchronousReply(action, request)
                except Exception as e:
                    payload = {"ok": False, "error": f"udp parse failed: {type(e).__name__}: {e}"}
                reply = json.dumps(payload, ensure_ascii=False, default=str).encode("utf-8")
                self.udp.writeDatagram(reply, host, port)
                if self.verbose:
                    print(f"[BRIDGE-SERVICE:verbose] udp resp to={host.toString()}:{port} bytes={len(reply)}", file=sys.stderr, flush=True)
        except Exception as udpErr:
            recordException("supergrok_bridge/app.py:_readUdpDatagrams", udpErr)

    def buildSynchronousReply(self, action: str, request: dict[str, Any]) -> dict[str, Any]:
        """Minimal sync replies for UDP-safe actions: status, ping, version.
        Returns the same shape the TCP handler returns for these actions."""
        if action in ("status", "ping", "health"):
            return {
                "ok": True,
                "action": action,
                "service": "supergrok-bridge",
                "host": BRIDGE_SERVICE_HOST,
                "port": int(self.server.serverPort()),
                "uptimeSeconds": round(time.time() - self.startedAt, 1),
                "pid": int(os.getpid()),
            }
        if action == "version":
            return {"ok": True, "version": "bridge-server", "pid": int(os.getpid())}
        return {"ok": False, "error": f"action '{action}' is not supported over UDP"}
    ...
    def readSocket(self, socket: QTcpSocket) -> None:
        ...
        try:
            ...
            if not chunk:
                return
            if self.verbose:
                preview = chunk[:200].decode("utf-8", "replace").replace("\n", "\\n").replace("\r", "\\r")
                print(f"[BRIDGE-SERVICE:verbose] recv key={key} bytes={len(chunk)}  preview={preview!r}", file=sys.stderr, flush=True)
            data = self.buffers.get(key, b"") + chunk
            # Detect HTTP framing on the very first bytes of this connection so
            # `curl -X POST http://127.0.0.1:<port>/ -d '{"action":"status"}'`
            # works alongside raw newline-JSON (telnet/nc). HTTP methods we
            # accept: GET/POST/OPTIONS/HEAD. Once flagged HTTP, the socket
            # stays HTTP for its lifetime.
            if key not in self.httpSockets and key not in self.currentRequestIds:
                head = bytes(data[:8]).upper()
                if head.startswith(b"GET ") or head.startswith(b"POST ") or head.startswith(b"OPTIONS ") or head.startswith(b"HEAD "):
                    self.httpSockets.add(key)
            if key in self.httpSockets:
                # Parse HTTP request: read headers up to CRLF CRLF, then body
                # of Content-Length bytes. If incomplete, buffer and wait.
                if b"\r\n\r\n" not in data:
                    self.buffers[key] = data
                    return
                head_blob, _, body_blob = data.partition(b"\r\n\r\n")
                self.buffers[key] = b""
                try:
                    head_lines = head_blob.decode("utf-8", "replace").split("\r\n")
                    request_line = head_lines[0] if head_lines else ""
                    method = (request_line.split(" ", 1)[0] if request_line else "GET").upper()
                    hdrs: dict[str, str] = {}
                    ...
                    if method == "OPTIONS":
                        # CORS preflight — answer empty with permissive headers.
                        self._respondHttpRaw(socket, 204, b"", extra_headers={"Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "GET,POST,OPTIONS", "Access-Control-Allow-Headers": "Content-Type"})
                        return
                    content_length = int(hdrs.get("content-length", "0") or "0")
                    body = body_blob[:content_length] if content_length else b""
                    if not body and method == "GET":
                        # Default GET with no body → treat as status probe.
                        body = b'{"action":"status"}'
                    line_text = body.decode("utf-8", "replace").strip() or '{"action":"status"}'
                    if self.verbose:
                        print(f"[BRIDGE-SERVICE:verbose] http {method} key={key} body={line_text!r}", file=sys.stderr, flush=True)
                    self.handleLine(socket, line_text)
                except Exception as err:
                    self._respondHttpRaw(socket, 400, json.dumps({"ok": False, "error": f"http parse failed: {err}"}).encode("utf-8"))
                return
            # Raw newline-JSON path (telnet / nc / Python TCP client / supergrok-bridge.js)
            while b"\n" in data:
    ...
    def _respondHttpRaw(self, socket: QTcpSocket, status: int, body: bytes, extra_headers: dict[str, str] | None = None) -> None:
        """Write a complete HTTP/1.1 response and close. Used for curl clients."""
        try:
            reason = {200: "OK", 204: "No Content", 400: "Bad Request", 500: "Internal Server Error"}.get(status, "OK")
            hdrs = {
            ...
            if self.verbose:
                print(f"[BRIDGE-SERVICE:verbose] http resp key={id(socket)} status={status} body_chars={len(body)}", file=sys.stderr, flush=True)
            socket.write(wire)
            socket.flush()
            socket.disconnectFromHost()
        except Exception as e:
            recordException("supergrok_bridge/app.py:_respondHttpRaw", e)
    ...
    def handleLine(self, socket: QTcpSocket, line: str) -> None:
        ...
        if action == "chat":
            ...
            # VISION-PRIMARY 2026-05-19. The GPT-4o-vision pilot is now the
            # DEFAULT and only intended path (screenshot → GPT → act → read
            # reply — no DOM selectors, future-proof against any UI redesign).
            # The legacy DOM/selector path is opt-OUT only, for debugging:
            # pass usePilot=false OR via in {dom,selector,legacy}.
            usePilot = (request.get("usePilot", True) is not False) and via not in {"dom", "selector", "legacy"}
            if usePilot:
            ...
            return
        if action == "library":
            # Reuse the resident webview to navigate to /library and scrape.
            # Cheaper than a new Qt process AND avoids the cache-conflict that
            # bites parallel webviews against the same on-disk profile.
            target = normalizeChatTarget(request.get("target") or getattr(getattr(self.window, "config", None), "target", "chatgpt"))
            if target != "chatgpt":
                self.respond(socket, {"ok": False, "error": f"library action only supports target=chatgpt; got {target!r}"})
                return
            timeoutSeconds = max(10, int(request.get("timeoutSeconds") or 45))
            hydrateMs = max(500, int(request.get("hydrateMs") or 3500))
            sockKey = id(socket)
            self.pendingChatSockets.add(sockKey)
            def _libraryReply(payload: dict[str, Any], *, sock: QTcpSocket = socket, sockKey_: int = sockKey) -> None:
                self.pendingChatSockets.discard(sockKey_)
                self.respond(sock, payload)
            try:
                self.window.fetchChatGptLibraryAsync(_libraryReply, timeoutSeconds=timeoutSeconds, hydrateMs=hydrateMs)
            except Exception as error:
                self.pendingChatSockets.discard(sockKey)
    ...
    def respond(self, socket: QTcpSocket, payload: dict[str, Any], requestId: str | None = None) -> None:
        ...
        try:
            ...
            try:
                ...
                return
            # HTTP-framed reply for curl clients; raw newline-JSON for telnet/nc.
            body_bytes = json.dumps(payload, ensure_ascii=False, default=str).encode("utf-8")
            if key in self.httpSockets:
                self._respondHttpRaw(socket, 200, body_bytes)
                self.httpSockets.discard(key)
                if self.verbose:
                    print(f"[BRIDGE-SERVICE:verbose] respond http key={key} bytes={len(body_bytes)}", file=sys.stderr, flush=True)
                return
            line = body_bytes + b"\n"
            if self.verbose:
                preview = body_bytes[:200].decode("utf-8", "replace")
                print(f"[BRIDGE-SERVICE:verbose] respond tcp key={key} bytes={len(line)}  preview={preview!r}", file=sys.stderr, flush=True)
            socket.write(line)
            socket.flush()
...
class SuperGrokBridgeWindow(QMainWindow):
    # NOTE 2026-05-19: an earlier "block every show()" override was REVERTED
    # here — it hung the child. "offscreen-window mode" is a REAL window that
    # MUST be show()n so QtWebEngine creates its compositing surface; without
    # the surface the page never loads and the command server never binds
    # (err 10061). The window is kept off-screen / hidden-from-user by the
    # offscreen-window positioning machinery + revealForHumanRepair being a
    # no-op, NOT by suppressing show(). Do not re-add a show() block.
    ...
    def createGrokWebView(self) -> ManagedWebView:
        ...
        # Profile storage name MUST match login_bridge.buildLoginProfile's
        # name (SuperGrok{label}Profile). Different names = separate cookie
        # jars under the same setPersistentStoragePath, which silently logs
        # the user out after every login.
        profile = QWebEngineProfile(f"SuperGrok{providerLabel}Profile", self)
        if profileDir:
        ...
        # Clear stale Chromium locks from a prior crashed child. These are
        # what caused "Unable to move the cache: Access is denied (0x5)" /
        # "Could not open the quota database" / "Failed to reset the quota
        # database" — the child died on a locked profile, watchdog respawned,
        # taskkill churned, popup. Nuking the whole cache/ dir is safe (it's
        # regenerable) and frees the lock. Cookies live under storage/ and
        # are preserved so logins survive; we only remove storage-side LOCK
        # / lockfile / *.lock + the quota DB which Chromium rebuilds clean.
        try:
            import shutil as _sh
            if cachePath.exists():
                _sh.rmtree(cachePath, ignore_errors=True)
            if storagePath.exists():
                for _lock in storagePath.rglob("*"):
                    n = _lock.name.lower()
                    if n in ("lock", "lockfile") or n.endswith(".lock") or n.startswith("quotamanager"):
                        try:
                            _lock.unlink()
                        except Exception:  # swallow-ok: best-effort lock clear
                            pass
            print(f"[profile] cleared stale cache+locks under {path}", flush=True)
        except Exception as _lkerr:
            recordException("supergrok_bridge/app.py:profile-lock-clear", _lkerr, extra={"path": str(path)})
...
"""
    ...
    def fetchChatGptLibraryAsync(self, callback: Callable[[dict[str, Any]], None], *, timeoutSeconds: int = 45, hydrateMs: int = 3500) -> None:
        """Navigate the resident webview to chatgpt.com/library, wait for the
        SPA to hydrate, scrape the file list via runJavaScript, and call back
        with {ok, files, ...}. Caller is the bridge `library` action handler.

        DOM selectors are first-cut guesses; ChatGPT rotates classes often.
        Expect to refine after the first real round-trip against a logged-in
        session. Login wall is detected and reported as
        reason='login-required' so callers can prompt the user to sign in
        through SuperGrok's normal `--chatgpt` login flow.
        """
        page = self.grokView.page()
        started = time.monotonic()
        ...
        def reply(payload: dict[str, Any]) -> None:
            ...
            try:
                ...
                recordException("supergrok_bridge/app.py:fetchChatGptLibraryAsync.callback", error, extra={"handler": "callback"})

        def on_scrape_complete(value: object) -> None:
            try:
                if isinstance(value, str):
                    ...
                    parsed = {"ok": False, "reason": "scraper-bad-return", "raw": str(value)[:500]}
                reply(parsed)
            except Exception as error:
                reply({"ok": False, "reason": "scraper-parse-error", "error": f"{type(error).__name__}: {error}", "raw": str(value)[:500]})

        def run_scraper() -> None:
            try:
                page.runJavaScript(_LIBRARY_SCRAPER_JS, on_scrape_complete)
            except Exception as error:
                reply({"ok": False, "reason": "runjs-failed", "error": f"{type(error).__name__}: {error}"})
        ...
        def on_load_finished(ok: bool) -> None:
            ...
            state["loaded"] = True
            try:
                page.loadFinished.disconnect(on_load_finished)
            except Exception:
                pass
            if not ok:
                reply({"ok": False, "reason": "page-load-failed", "url": page.url().toString()})
                return
            QTimer.singleShot(max(0, int(hydrateMs)), run_scraper)

        def on_total_timeout() -> None:
            try:
                page.loadFinished.disconnect(on_load_finished)
            except Exception:
                pass
            reply({"ok": False, "reason": "timeout", "timeoutSeconds": int(timeoutSeconds), "url": page.url().toString()})

        page.loadFinished.connect(on_load_finished)
        page.load(QUrl("https://chatgpt.com/library"))
        QTimer.singleShot(max(1000, int(timeoutSeconds * 1000)), on_total_timeout)
    ...
    def kickCompositorForCli(self, reason: str = "") -> None:
        try:
            ...
            # Per user 2026-05-18: bridge window must NEVER be visible.
            # Chromium still needs a real opaque surface to composite, so
            # we keep opacity=1.0 + show(), but we park the window 20,000px
            # off the primary screen (negative coords work cross-monitor).
            # lower() + WA_ShowWithoutActivating keep it out of the taskbar
            # focus order. Net effect: page renders, cookies persist, user
            # sees nothing.
    ...
    def revealForHumanRepair(self, reason: str = "") -> None:
        """DISABLED 2026-05-19 per user (third time): the bridge window must
        NEVER become visible. Earlier disable-comments at app.py:5969 and
        login_bridge.py:241/342 covered other paths but this one was missed,
        and it KEPT popping the grok.com page on every DOM-probe failure.
        Body fully gutted — no show()/raise_()/setWindowOpacity(1.0)/move().
        We log the trigger so it's visible in the err log, then return.
        Future devs: do not re-enable. If a repair flow is needed, surface
        a message via the chat panel or status bar — never a Qt window."""
        warnLog(f"revealForHumanRepair suppressed (reason={reason!r}); bridge stays offscreen per user policy")
        try:
            self.debugPane.append("warn", f"revealForHumanRepair suppressed (reason={reason!r})")
        except Exception:  # swallow-ok: debug pane may not exist in headless mode
            pass
...
def runApplication(initialUrl: str, target: str = "grok", debug: bool = False, profileDir: str = "", remoteDebugPort: int = 9222, processTtlSeconds: int = PROCESS_DEFAULT_TTL_SECONDS, serviceMode: bool = False, servicePort: int = BRIDGE_SERVICE_PORT, hideWindow: bool = False, windowMode: str = "visible", offscreenMode: str = "auto") -> int:
    # Re-enabled 2026-05-19. The window can no longer pop: SuperGrokBridgeWindow
    # overrides show/showNormal/showMaximized/showFullScreen/setVisible to
    # force-hide, revealForHumanRepair is a no-op, and Tasks.taskkill no longer
    # shells out (no 0xc0000142). Headless chat round-trip runs; nothing paints.
    ...
    def showWindow() -> None:
        # UN-BURNED 2026-05-19. The earlier burn was a panic reaction to the
        # popup spam, not the design. Confirmed intent: the window MUST exist
        # and render (NN chrome + QtWebEngine) so grab() produces a real
        # screenshot for GPT-vision — it just must be invisible to the user.
        # The round-6 invisible-but-rendering stack below (Tool flag +
        # opacity 0.0 + move 5000,5000 + stays-on-bottom + lower) is the one
        # config where Chromium keeps rasterizing AND nothing is visible.
        window = state["window"]
        config = state["config"]
        ...
        if mode == "offscreen-window":
            ...
            if os.name == "nt":
                # 2026-05-19 round 6 (ported verbatim from the battle-tested
                # C:\SuperGrok\app.py): WA_DontShowOnScreen blanks Chromium's
                # renderer (grab() returns white PNGs — Chromium never
                # rasterizes a never-displayed widget), and move(-32000)
                # makes Chromium SUSPEND rendering ("fully off-screen on all
                # monitors"). The window MUST actually be shown for Chromium
                # to keep producing frames for the GPT-vision pilot. Make it
                # invisible by stacking instead:
                #   - Tool flag                 → no taskbar entry
                #   - opacity 0.0               → user can't see it on-screen
                #   - move(5000,5000)           → past monitor edges, NOT -32000
                #   - WindowStaysOnBottom+lower → z-order below everything
                # Net: Chromium renders normally (grab() captures pixels for
                # the vision pilot), the user sees absolutely nothing.
                try:
                    ...
                    recordException("supergrok_bridge/app.py:offscreen-window-attrs", error, extra={"mode": mode})
                try:
                    window.resize(1280, 900)
                    if hasattr(window, "mainSplitter"):
                        window.mainSplitter.setSizes([0, 0, 1280])
                    # Collapse chrome so QWidget.grab() is mostly just the
                    # web view — the vision pilot needs a tall enough capture
                    # to see login forms, not a 232px sliver.
                    for attr in ("menuBar", "statusBar"):
                        try:
                            ...
                            pass
                    for attr in ("requestPane", "addressBar", "addressBarRow", "grokToolbar"):
                        try:
                ...
                try:
                    window.lower()
                    QTimer.singleShot(150, window.lower)
                    QTimer.singleShot(600, window.lower)
                except Exception:  # swallow-ok
                    pass
            else:
